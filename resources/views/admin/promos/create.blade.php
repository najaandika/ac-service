...
<div class="space-y-6">
    <!-- Page Header -->
    <x-page-header title="Tambah Promo Baru" subtitle="Buat kode promo untuk pelanggan" />
    ...
    <form action="{{ route('admin.promos.store') }}" method="POST" class="space-y-6">
        ...
        <x-cards.card title="Pengaturan Diskon">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6" x-data="promoDiscountForm" data-initial-type="{{ old('type', 'percentage') }}">
                <x-forms.select 
                    name="type" 
                    label="Tipe Diskon" 
                    :required="true"
                ...
                <div>
                    ...
                    <template x-if="discountType === 'percentage'">
                        <div class="flex">
                            ...
                            <input type="number" name="value" id="value" value="{{ old('value') }}" min="1" max="100" step="any" class="form-input w-full rounded-l-none @error('value') border-red-500 @enderror" placeholder="10" required autocomplete="off">
                        </div>
                    </template>
                    
                    <template x-if="discountType === 'fixed'">
                        <x-forms.currency-input name="value" placeholder="25.000" :value="old('value')" />
                    </template>
                ...
                <div>
                    <x-forms.currency-input name="min_order" label="Minimum Order" placeholder="100.000" :value="old('min_order')" />
                    <p class="text-xs text-gray-500 -mt-3">Kosongkan jika tanpa minimum order</p>
                </div>
                
                <div x-show="discountType === 'percentage'">
                    <x-forms.currency-input name="max_discount" label="Maksimum Potongan" placeholder="50.000" :value="old('max_discount')" />
