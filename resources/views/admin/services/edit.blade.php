        ...
        <x-cards.card title="Informasi Dasar">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="name" value="{{ old('name', $service->name) }}" class="form-input w-full @error('name') border-red-500 @enderror" placeholder="Contoh: Cuci AC" required>
                    @error('name')
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="3" class="form-input w-full @error('description') border-red-500 @enderror" placeholder="Jelaskan layanan ini..." required>{{ old('description', $service->description) }}</textarea>
                    @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="duration_minutes" value="{{ old('duration_minutes', $service->duration_minutes) }}" min="15" class="form-input w-full @error('duration_minutes') border-red-500 @enderror" required>
        ...
        <x-cards.card title="Fitur Layanan">
            ...
            <div class="space-y-3" id="features-container">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <input type="text" name="features[]" value="{{ $features[$i] ?? '' }}" class="form-input flex-1" placeholder="Contoh: Pengecekan kebocoran pipa">
                </div>
                @endfor
            </div>
        </x-cards.card>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div class="bg-gray-50 rounded-lg p-4">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" inputmode="numeric" name="prices[{{ $cap }}]" value="{{ old('prices.' . $cap, number_format($priceMap[$cap] ?? 0, 0, ',', '.')) }}" placeholder="0" class="form-input w-full !pl-12 @error('prices.' . $cap) border-red-500 @enderror">
                    </div>
                </div>
                @endforeach
            </div>
